import React, {useState, useEffect} from 'react';
import {View, StyleSheet, ScrollView, ActivityIndicator} from 'react-native';
import {ImageSlider} from 'react-native-image-slider-banner';
import {getDatabase, onValue, ref, set} from '@firebase/database';

import {theme} from '../global/styles/theme';
import TitleSection from '../components/TitleSection';
import GrayText from '../components/GrayText';
import ButtonPrimary from '../components/ButtonPrimary';
import ButtonSecondary from '../components/ButtonSecondary';

export default function ProductInfo({navigation, route}) {
  const [value, setValue] = useState(0);
  const [amount, setAmount] = useState([]);
  const [product, setProduct] = useState();
  const [loading, setLoading] = useState(false);

  const db = getDatabase();

  const getProduct = () => {
    const docRef = (db, 'products/' + '6e4776c1-2d61-466b-bd7c-dea519400730');
    onValue(docRef, snapshot => {
      const data = snapshot.val();
      setProduct(data);
    });
  };

  const images = [
    {
      img: 'https://img.itdg.com.br/tdg/images/blog/uploads/2017/08/shutterstock_279701309-300x200.jpg',
    },
    {
      img: 'https://alavoura.com.br/wp-content/uploads/2020/08/0-shutterstock_194249981.jpg',
    },
    {
      img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrJsqh-Uxw_GU2U9jxeJOSvyXWRtJ0BMDGX7REoXSly4WzwS6WpEeGuFgnDdoA7yOuVrg&usqp=CAU',
    },
    {
      img: 'data:image/jpeg;base64,',
    },
  ];

  useEffect(() => {
    getProduct();
  }, []);

  return (
    <ScrollView>
      <ImageSlider
        data={images}
        closeIconColor={theme.pallete.primary005}
        indicatorContainerStyle={{
          color: theme.pallete.primary005,
        }}
        inActiveIndicatorStyle={{
          color: theme.pallete.primary005,
        }}
        activeIndicatorStyle={{
          color: theme.pallete.primary005,
        }}
      />
      <View style={styles.whiteArea} showsVerticalScrollIndicator={true}>
        {product ? (
          <ActivityIndicator color={theme.pallete.primary006} />
        ) : (
          <>
            <TitleSection>product.name</TitleSection>
            <GrayText>product.price (product.formsOfSale)</GrayText>
            <View style={{marginTop: 16}} />
            <TitleSection>Descrição</TitleSection>
            <GrayText>product.description</GrayText>
            <View style={{marginTop: 16}} />
            <TitleSection>Próxima entrega</TitleSection>
            <GrayText>16 de outubro (terça-feira)</GrayText>
            <View style={{marginTop: 16}} />
            <TitleSection>Local de entrega</TitleSection>
            <GrayText>product.placeOfSale</GrayText>
            <View style={{marginTop: 64}} />
            <ButtonPrimary>COMPRAR</ButtonPrimary>
            <ButtonSecondary>ADICIONAR À CESTA</ButtonSecondary>
          </>
        )}
      </View>
    </ScrollView>
  );
}

const styles = StyleSheet.create({
  whiteArea: {
    height: '100%',
    backgroundColor: theme.pallete.white,
    padding: 8,
    alignItems: 'center',
  },
});
